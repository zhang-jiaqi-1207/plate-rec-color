...
### Import this for type hint
from rknn.api import RKNN
from torch import device
from numpy import ndarray


def cv_imread(path: str) -> ndarray:  #可以读取中文路径的图片
...
def image_processing_rknn(img, device):
    img = cv2.resize(img, (168,48))
    img = np.reshape(img, (48, 168, 3))
...
def get_plate_result_rknn(img: ndarray,device: device, model: RKNN, is_color: bool=False):
    ...
    if is_color:  #是否识别颜色
        ...
        preds = model.inference(inputs=[input])
