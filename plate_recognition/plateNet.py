...
class myNet_ocr_color2(nn.Module):
    def __init__(self,cfg=None,num_classes=78,export=False,color_num=None):
        super(myNet_ocr_color2, self).__init__()
        if cfg is None:
            cfg =[32,32,64,64,'M',128,128,'M',196,196,'M',256,256]
            # cfg =[32,32,'M',64,64,'M',128,128,'M',256,256]
        self.feature = self.make_layers(cfg, True)
        ...
        if self.color_num:
            ...
            # self.color_bn = nn.BatchNorm2d(self.color_num)
            # self.flatten = nn.Flatten()
            self.conv0 = nn.Conv2d(3, 64, kernel_size=5, stride=2)
            self.bn0 = nn.BatchNorm2d(64)
            self.relu0 = nn.ReLU(inplace=True)
            self.conv1 = nn.Conv2d(64, 128, kernel_size=3, stride=1)
            self.bn1 = nn.BatchNorm2d(128)
            self.relu1 = nn.ReLU(inplace=True)
            self.pool1 = nn.MaxPool2d(kernel_size=3, stride=1, ceil_mode=True)
            self.conv2 = nn.Conv2d(128,64,kernel_size=3,stride=1)
            self.bn2 = nn.BatchNorm2d(64)
            self.relu2 = nn.ReLU(inplace=True)
            self.pool2 = nn.AdaptiveAvgPool2d(output_size=1)
            self.flatten = nn.Flatten()
            self.fc = nn.Linear(64, self.color_num)
        self.loc =  nn.MaxPool2d((5, 2), (1, 1),(0,1),ceil_mode=False)
        self.newCnn=nn.Conv2d(cfg[-1],num_classes,1,1)
        # self.newBn=nn.BatchNorm2d(num_classes)
    def make_layers(self, cfg, batch_norm=False):
    ...
    def forward(self, xin):
        ...
        if self.color_num:
            ...
            # x_color =self.gap(x_color)
            # x_color = self.flatten(x_color)
            x_color = self.conv0(xin)
            x_color = self.bn0(x_color)
            x_color = self.relu0(x_color)
            x_color = self.conv1(x_color)
            x_color = self.bn1(x_color)
            x_color = self.relu1(x_color)
            x_color = self.pool1(x_color)
            x_color = self.conv2(x_color)
            x_color = self.bn2(x_color)
            x_color = self.relu2(x_color)
            x_color = self.pool2(x_color)
            x_color = self.flatten(x_color)
            x_color = self.fc(x_color)
        x=self.loc(x)
        x=self.newCnn(x)
