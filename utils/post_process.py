...
from plate_recognition.plate_rec import get_plate_result, get_plate_result_rknn
from plate_recognition.double_plate_split_merge import get_split_merge


### Import these module/class for type hint
from typing import Any, Dict, List, Union
# from typing_extensions import List, Dict, Union       ## if your python interpreter version is too new, use this
...
def load_model(weights: str, device: device) -> Ensemble:   #加载检测模型
...
def get_plate_rec_landmark(img: ndarray,
                           xyxy: List, 
                           conf: ndarray, 
                           landmarks: List, 
                           class_num: ndarray,
                           device: device,
                           plate_rec_model: Union[myNet_ocr_color, Any],
                           is_color: bool=False) -> Dict:  #获取车牌坐标以及四个角点坐标并获取车牌号
    """
    Description: 
    """
    h,w,c = img.shape
...
def get_plate_rec_landmark_rknn(img: ndarray, 
                                xyxy: List, 
                                conf: ndarray, 
                                landmarks: List, 
                                class_num: ndarray,
                                device: device,
                                plate_rec_model: RKNN,
                                is_color: bool=False) -> Dict:  #获取车牌坐标以及四个角点坐标并获取车牌号
    """
    Description:
    """
    ...
    if class_label:        #判断是否是双层车牌，是双牌的话进行分割后然后拼接
        roi_img=get_split_merge(roi_img)
    if not is_color:
        plate_number,rec_prob = get_plate_result_rknn(roi_img,device,plate_rec_model,is_color=is_color)                 #对车牌小图进行识别
    else:
        plate_number,rec_prob,plate_color,color_conf=get_plate_result_rknn(roi_img,device,plate_rec_model,is_color=is_color) 
    # cv2.imwrite("roi.jpg",roi_img)
    result_dict['rect']=rect                      #车牌roi区域
    result_dict['detect_conf']=conf              #检测区域得分
    result_dict['landmarks']=landmarks_np.tolist() #车牌角点坐标
...
def detect_Recognition_plate(model: Union[Module, ModuleList, Ensemble], 
                             orgimg: ndarray, 
                             device: device, 
                             plate_rec_model: Union[myNet_ocr_color, Any], 
                             img_size: int, 
                             is_color: bool=False) -> List:
    """
...
def detect_Recognition_plate_rknn(model: Union[Module, ModuleList, Ensemble], 
                                  orgimg: ndarray, 
                                  device: device,
                                  plate_rec_model: RKNN, 
                                  img_size: int, 
                                  is_color: bool=False) -> List:
    """
    Description : 获取车牌信息。先经过pt文件检测 再通过plate_rec_model(rknn model)识别车牌信息
    """
    conf_thres = 0.3      #得分阈值
    iou_thres = 0.5       #nms的iou值   
    dict_list=[]
    ...
    for i, det in enumerate(pred):  # detections per image
        if len(det):
            ...
            for j in range(det.size()[0]):
                ...
                landmarks = det[j, 5:13].view(-1).tolist()
                class_num = det[j, 13].cpu().numpy()
                result_dict = get_plate_rec_landmark_rknn(orgimg, xyxy, conf, landmarks, class_num,device,plate_rec_model,is_color=is_color)
                dict_list.append(result_dict)
    return dict_list

def draw_result(orgimg: ndarray, 
                dict_list: List,
                is_color: bool=False) -> ndarray:   # 车牌结果画出来
    """
